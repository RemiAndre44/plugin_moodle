 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_multichoice_advance_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $elename = 'multichoice_advance';
        // We used get_recommended_name() so this works.
        $elepath = $this->get_pathfor('/multichoice_advance');
    ...
    /**
     * Process the qtype/multichoice_advance element
     */
    public function process_multichoice_advance($data) {
        global $DB;
        ...
        // qtype_multichoice_advance_options too.
        if ($questioncreated) {
            $data->questionid = $newquestionid;

            // It is possible for old backup files to contain unique key violations.
            // We need to check to avoid that.
            if (!$DB->record_exists('qtype_multichoice_advance', array('questionid' => $data->questionid))) {
                $newitemid = $DB->insert_record('qtype_multichoice_advance', $data);
                $this->set_mapping('qtype_multichoice_advance', $oldid, $newitemid);
            }
        }
    }
    ...
    public static function define_decode_contents() {
        ...
        $contents[] = new restore_decode_content('qtype_multichoice_advance',
                $fields, 'qtype_multichoice_advance');
